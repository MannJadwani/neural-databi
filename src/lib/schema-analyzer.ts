...
export function analyzeSchema(schema: DatasetSchema): VisualizationSuggestion[] {
  ...
  if (dateColumns.length > 0 && numericColumns.length > 0) {
    ...
    if (numericColumns.length === 1) {
      suggestions.push({
        chartType: 'line',
        title: `${numericColumns[0].name} over ${dateCol.name}`,
        reasoning: `Time series: "${dateCol.name}" (date) × "${numericColumns[0].name}" (numeric)`,
        config: { xAxis: dateCol.name, yAxis: numericColumns[0].name },
        confidence: 0.9,
      });
    } else {
      ...
      suggestions.push({
        ...
        reasoning: `Time series with ${numericColumns.length} metrics over "${dateCol.name}"`,
        config: { xAxis: dateCol.name, yAxis: numericColumns.slice(0, 4).map((c) => c.name), showLegend: true },
        confidence: 0.85,
      });
    }
    ...
    suggestions.push({
      ...
      reasoning: `Area view of "${numericColumns[0].name}" over "${dateCol.name}"`,
      config: { xAxis: dateCol.name, yAxis: numericColumns[0].name },
      confidence: 0.7,
    });
  }
  ...
  for (const cat of categoricalColumns) {
    for (const num of numericColumns.slice(0, 2)) {
      suggestions.push({
        ...
        config: { xAxis: cat.name, yAxis: num.name },
        confidence: 0.8,
      });
    }
  }
  ...
  for (const cat of categoricalColumns.filter((c) => isLowCardinality(c, 8))) {
    if (numericColumns.length > 0) {
      suggestions.push({
        ...
        reasoning: `Proportional view: "${cat.name}" has only ${cat.uniqueCount} categories — good for pie/donut`,
        config: { xAxis: cat.name, yAxis: numericColumns[0].name },
        confidence: 0.75,
      });
      break; // Only one pie suggestion
    }
  }
